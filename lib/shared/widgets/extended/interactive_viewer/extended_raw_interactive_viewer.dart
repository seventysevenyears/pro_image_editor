...
class ExtendedTransformationController extends ValueNotifier<Matrix4> {
  /// Create an instance of [ExtendedTransformationController].
  ///
  /// The [value] defaults to the identity matrix, which corresponds to no
  /// transformation.
  ExtendedTransformationController([Matrix4? value])
...
/// See also:
///
///   * [ExtendedRawInteractiveViewer.builder], whose builder is of this type.
...
/// {@end-tool}
class ExtendedRawInteractiveViewer extends StatefulWidget {
  /// Create an InteractiveViewer.
  ExtendedRawInteractiveViewer({
    super.key,
  ...
  /// large child.
  ExtendedRawInteractiveViewer.builder({
    super.key,
    this.clipBehavior = Clip.hardEdge,
    this.panAxis = PanAxis.free,
    this.boundaryMargin = EdgeInsets.zero,
  ...
  ///
  /// Passed with the [ExtendedRawInteractiveViewer.builder] constructor. Otherwise, the
  ...
  ///
  /// If the [ExtendedRawInteractiveViewer.builder] constructor is used, then this will be
  ...
  ///
  /// At the time this is called, the [ExtendedTransformationController] will have
  ...
  ///
  /// At the time this is called, the [ExtendedTransformationController] will not have
  ...
  /// [ExtendedTransformationController.toScene] for how to convert these coordinates to
  ...
  /// A [ExtendedTransformationController] for the transformation performed on the
  /// child.
  ///
  ...
  final ExtendedTransformationController? transformationController;
  ...
  static Vector3 getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final List<Vector3> closestPoints = <Vector3>[
      ExtendedRawInteractiveViewer.getNearestPointOnLine(
        point,
        quad.point0,
        quad.point1,
      ),
  ...
  @override
  State<ExtendedRawInteractiveViewer> createState() =>
      ExtendedRawInteractiveViewerState();
}

/// The state class for the [ExtendedRawInteractiveViewer] widget.
///
/// This class manages the interactive state and behavior of the
/// extended raw interactive viewer, including gesture handling,
/// transformation logic, and rendering updates.
///
/// Typically used internally by [ExtendedRawInteractiveViewer] to
/// provide advanced image or content manipulation features such as
/// zooming, panning, and rotation.
class ExtendedRawInteractiveViewerState
    extends State<ExtendedRawInteractiveViewer>
    with TickerProviderStateMixin {
  late ExtendedTransformationController _transformer =
      widget.transformationController ?? ExtendedTransformationController();
  ...
  void didUpdateWidget(ExtendedRawInteractiveViewer oldWidget) {
    ...
    _transformer = newController ?? ExtendedTransformationController();
    _transformer.addListener(_handleTransformation);
  }
...
Quad _getAxisAlignedBoundingBoxWithRotation(Rect rect, double rotation) {
  ...
  return ExtendedRawInteractiveViewer.getAxisAlignedBoundingBox(
    boundariesRotated,
  );
}
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside =
        ExtendedRawInteractiveViewer.getNearestPointInside(point, boundary);
